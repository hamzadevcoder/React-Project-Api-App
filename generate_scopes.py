...
def get_dashboard_type(category, name):
    if name in ["pages_messaging", "instagram_manage_messages"]:
        return "messaging"
    return dashboard_mapping.get(category, "generic")

def get_allowed_usage(category, name):
    if category == "Ads":
        return ["Programmatically create campaigns and manage ads", "Fetch ad performance metrics", "Build ad management tools"]
    elif category == "Instagram":
        return ["Read an Instagram user's profile and media", "Publish content to Instagram", "Reply to comments and messages"]
    elif category == "Pages":
        return ["Read page content and posts", "Manage page settings and webhooks", "Publish as the Page"]
    elif category == "User":
        return ["Read user's basic demographic data", "Read user's social graph", "Personalize user experience"]
    else:
        return [f"Enable features related to {name.replace('_', ' ')}", "Read necessary data metrics", "Manage related business assets"]

def generate_scopes():
    scopes = []
    
    for category, names in scopes_def.items():
        for name in names:
            d_type = get_dashboard_type(category, name)
            
            # Create a mock example response based on d_type
            example_resp = {"data": []}
            endpoint = "/v25.0/me"
            ...
            scope = {
                ...
                "name": name,
                "category": category,
                "accessLevel": "Advanced" if "manage" in name else "Standard",
                "description": f"Allows your app to interact with the {name} API.",
                "allowedUsage": get_allowed_usage(category, name),
                "dependencies": ["public_profile"] if name != "public_profile" else [],
                "endpoint": endpoint,
                "curlExample": f'curl -X GET "https://graph.facebook.com{endpoint.split(" ")[1] if len(endpoint.split(" "))>1 else endpoint}?access_token=YOUR_TOKEN"',
                "exampleResponse": example_resp,
                "dashboardType": d_type,
                "metaDocsUrl": f"https://developers.facebook.com/docs/permissions#{name}"
            }
            scopes.append(scope)
...
with open('src/data/scopes.js', 'w') as f:
    f.write('export const scopes = ')
    json.dump(generate_scopes(), f, indent=2)
    f.write(';\n')
